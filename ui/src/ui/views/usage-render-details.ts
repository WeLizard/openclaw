...
function renderSessionSummary(
  ...
  if (!usage) {
    return html`
      <div class="muted">${t("usage.noUsageDataForSession")}</div>
    `;
  }
  ...
  return html`
    ...
    <div class="session-summary-grid">
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${t("usage.userAssistantCounts", {
      ...
      <div class="session-summary-card">
        ...
        <div class="session-summary-meta">${t("usage.toolsCount", { count: String(uniqueToolCount) })}</div>
      </div>
      <div class="session-summary-card">
        <div class="session-summary-title">${t("usage.errors")}</div>
        <div class="session-summary-value">${usage.messageCounts?.errors ?? 0}</div>
        <div class="session-summary-meta">${t("usage.toolResults", { count: String(usage.messageCounts?.toolResults ?? 0) })}</div>
      </div>
      <div class="session-summary-card">
      ...
      <div class="session-detail-header">
        <div class="session-detail-header-left">
          <div class="session-detail-title">
            ${displayLabel}
            ${cursorIndicator ? html`<span style="font-size: 11px; color: var(--muted); margin-left: 8px;">${t("usage.filteredTag")}</span>` : nothing}
          </div>
        </div>
        ...
        <div class="timeseries-controls">
          ${
            ...
            <div class="chart-toggle small">
              <button class="toggle-btn active" @click=${() => onCursorRangeChange?.(null, null)}>${t("common.reset")}</button>
            </div>
          `
              : nothing
          }
  ...
  return html`
    <div class="session-logs-compact">
      <div class="session-logs-header">
        <span>${t("usage.conversation")} <span style="font-weight: normal; color: var(--muted);">(${displayedCount} ${t("usage.messages")})</span></span>
      ...
      <div class="session-logs-list">
        ${filteredEntries.map((entry) => {
          ...
          const roleLabel =
            log.role === "user" ? t("usage.you") : log.role === "assistant" ? t("usage.assistant") : t("usage.tool");
          return html`
        ...
        ${
          filteredEntries.length === 0
            ? html`
                <div class="muted" style="padding: 12px">${t("usage.noMessagesMatchFilters")}</div>
              `
            : nothing
        }
      </div>
