...
function createCopyButton(options: CopyButtonOptions): TemplateResult {
  const idleLabel = options.label ?? t("common.copyAsMarkdown");
  const copiedLabel = t("common.copied");
  const errorLabel = t("common.copyFailed");
  return html`
    <button
      class="chat-copy-btn"
      type="button"
...
export function renderCopyAsMarkdownButton(markdown: string): TemplateResult {
  return createCopyButton({ text: () => markdown, label: t("common.copyAsMarkdown") });
}
